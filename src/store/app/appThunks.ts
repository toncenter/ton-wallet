...
export const createWalletContract = createAsyncThunk(
    ...
    withError<void, any, any>(async (empty, thunkAPI) => {
        ...
        const walletClass = walletVersion
            ? tonWebService.ton.wallet.all[walletVersion]
            : tonWebService.ton.wallet.default;
        const walletContract = new walletClass(tonWebService.ton.provider, {
...
export const updateWallet = createAsyncThunk(
    ...
    withError<void, any, any>(async (empty, thunkAPI) => {
        ...
        const isBalanceChanged =
            state.app.balance === null || new TonWeb.utils.BN(state.app.balance).cmp(balance) !== 0;
...
export const importWallet = createAsyncThunk(
    ...
    withError<string[], any, any>(async (myMnemonicWords) => {
        ...
        for (const WalletClass of tonWebService.ton.wallet.list) {
...
export const getFees = createAsyncThunk(
    'app/wallet/fee',
    withError<{ address: string; amount: string; comment: string }, any, any>(
        async ({ address, amount, comment }, thunkAPI) => {
            const state = thunkAPI.getState() as RootState;
...
export const walletSend = createAsyncThunk(
    'app/wallet/send',
    withError<{ address: string; amount: string; comment: string; words?: string[] }, any, any>(
        async ({ address, amount, comment, words }, thunkAPI) => {
            try {
                const state = thunkAPI.getState() as RootState;
                ...
                if (state.app.isLedger) {
                    ...
                    const query = await ledgerApp.transfer(
                        ACCOUNT_NUMBER,
                        tonWebService.walletContract,
                        address,
                        amount,
                        seqno,
                        addressFormat,
...
export const showAddressOnDevice = createAsyncThunk(
    ...
    withError<void, any, any>(async (empty, thunkAPI) => {
        ...
        const { address } = await ledgerApp.getAddress(
            ACCOUNT_NUMBER,
            true,
            ledgerApp.ADDRESS_FORMAT_USER_FRIENDLY +
                ledgerApp.ADDRESS_FORMAT_URL_SAFE +
                ledgerApp.ADDRESS_FORMAT_BOUNCEABLE,
        );
